...
# --- CORE FUNCTION: TIPS GENERATION (Existing Fixed Logic) ---
...
def generate_resume_tips(name, email, phone, match_score, current_skills_set, required_skills_set):
    ...
    for i, tip in enumerate(raw_tips):
        # Use the bolded part of the tip as the uniqueness check
        if tip.split('**')[1] not in unique_tips:
            unique_tips.add(tip.split('**')[1])
            numbered_tips.append(f"**{len(numbered_tips) + 1}.** {tip}")
...
if st.sidebar.button("Use Test Resume: JOHN DOE"):
    # Ensure the test resume has many skills to test the display fix
    test_resume = create_in_memory_resume("JOHN DOE",
                                          "Python, Java, C++, SQL, Machine Learning, Streamlit, React, HTML, CSS, Git, Docker, AWS")
    ...
    for file in files_to_process:
        ...
        try:
            ...
            result = {
                ...
                "Email": contact['email'],
                "Phone": contact['phone'],
                "Skills Found": ", ".join(raw_skills),  # FIX: Ensure skills are joined by ", "
