...
try:
    # Use try-except block in case the model is not installed (prevents full crash)
    nlp = spacy.load('en_core_web_sm')
except Exception:
    nlp = None
    print("Warning: Spacy model 'en_core_web_sm' not loaded. Name extraction may be less accurate.")
...
def extract_name(text):
    """
    FIXED: Prioritizes All-Caps AND Title-Case names at the top to fix 'Unknown Candidate'.
    """
    ...
    for line in lines[:10]:
        ...
        if stripped_line:
            # FIX: Check for All-Caps OR Title-Case (The original code only checked for is_all_caps)
...
def extract_skills(text, skills_db):
    """
    Extract skills using simple set intersection. (No change needed here)
    """
    if not nlp: return []
