...
import streamlit as st  # ADDED: Import Streamlit for caching


# --- Load Spacy Model using Streamlit Caching ---
# Caching ensures the model is loaded only once and handles resource management.

@st.cache_resource
def load_spacy_model():
    """Loads the Spacy model and handles the download/load failure."""
    try:
        # We try to load the model. If not found, it's a deployment issue.
        return spacy.load('en_core_web_sm')
    except Exception as e:
        # In a deployment environment, if the model fails to load,
        # we log the error and return None, which will trigger the
        # return [] in extract_skills, but with a warning.
